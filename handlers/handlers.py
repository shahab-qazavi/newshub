...
class News(BaseHandler):
    ...
    def after_get_one(self, document):
        ...
        result = self.prepare_dataset(col_news.find({'category_id': document['category_id'], 'text': {'$ne': ''}},
                                                    {'image': 1, 'title': 1, 'summary': 1, 'create_date': 1, '_id':1}).limit(12))
    ...
    def before_get(self):
            try:
                ...
                if self.params.get('header') == 'header':
                    print(1)
                    # print()
                    news = self.prepare_dataset(col_news.find({'text': {'$ne': ''}}).limit(1))
                    print(news[0])
                    ...
                    # print(1)
                    news = self.prepare_dataset(col_news.find().limit(8))
                    col_sources = self.db['sources']
