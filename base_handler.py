...
24- Data range condition
25- Python 3
26- fix Logical delete bug to show for admin
27- fix python3 problem with strings and byte arrays
28- fix bug for binary params
29- prepare_dataset now uses prepare item
'''
import inspect
import json
...
class BaseHandler(RequestHandler):
    ...
    def set_output(self, group, id, data=None):
        try:
            # print(group)
            # print(id)
            # print(consts.MESSAGES[group][id])
    ...
    def kmwrite(self):
        # print('role: %s user_id: %s' % (self.user_role, self.user_id))
            ...
            if self.token is None:
                ...
                if token_info is None:
                    ...
                    if datetime.now() > (tokan_date -timedelta(seconds=-consts.MAX_TOKEN_DURATION)):
                        ...
                        print(token_info)
                        self.premium = True if token_info.get('premium') == 1 else False
    ...
    def load_params(self):
        ...
        try:
            if self.method == '':
                ...
                self.params = json.loads(self.request.body.decode())
                self.original_params = deepcopy(self.params)
                    ...
                    if self.get_validation_check():
                        if self.data_casting():
                            if 'conditions' in self.params:
                                ...
                                if 'date_range_condition' in self.conditions.keys():
                                    self.conditions[self.conditions['date_range_condition']['field']] = {
                                        '$gte': self.conditions['date_range_condition']['from'],
                                         '$lte': self.conditions['date_range_condition']['to']}
                                    del self.conditions['date_range_condition']
                                    # del self.conditions['id_list']
